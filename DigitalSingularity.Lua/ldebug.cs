...
public static unsafe partial class Lua
{
    ...
    /// </summary>
    public static void lua_sethook(lua_State* L, lua_Hook func, byte mask, int count)
    {
        if (func == null || mask == 0)
        {
            // turn off hooks?
    ...
    public static byte lua_gethookmask(lua_State* L)
    {
        return L->hookmask;
    }
    ...
    public static bool lua_getstack(lua_State* L, int level, ref lua_Debug ar)
    {
        if (level < 0)
        {
    ...
    {
        if ((clLvalue(s2v(ci->func.p))->p->flag & PF_VAHID) != 0)
        {
    ...
    public static string? lua_getlocal(lua_State* L, int n)
    {
        lua_lock(L);
    ...
    public static string? lua_getlocal(lua_State* L, ref lua_Debug ar, int n)
    {
        lua_lock(L);
    ...
    public static string? lua_setlocal(lua_State* L, ref lua_Debug ar, int n)
    {
        lua_lock(L);
        StkId pos = null; // to avoid warnings
        string? name = luaG_findlocal(L, ar.i_ci, n, &pos);
    ...
    private static void collectvalidlines(lua_State* L, Closure* f)
    {
        ...
        else
        {
            ...
            if (p->lineinfo != null)
            {
                ...
                {
                    // vararg function
                    Debug.Assert(GET_OPCODE(p->code[0]) == OpCode.VarArgPrep);
    ...
    public static bool lua_getinfo(lua_State* L, string what, ref lua_Debug ar)
    {
