...
//   Debug.Assert(GET_OPCODE(*jmp) == OP_JMP);
...
//   Debug.Assert(getOpMode(o) == ivABC);
//   Debug.Assert(A <= MAXARG_A && B <= MAXARG_vB &&
//              C <= MAXARG_vC && (k & ~1) == 0);
...
//   int b = Bc + OFFSET_sBx;
//   Debug.Assert(getOpMode(o) == iAsBx);
...
//       break;
//     }
//     default: Debug.Assert(0);  /* invalid var kind to store */
//   }
//   freeexp(fs, ex);
// }
//
...
//   Debug.Assert(testTMode(GET_OPCODE(*pc)) && GET_OPCODE(*pc) != OP_TESTSET &&
...
//       break;
//     }
//     default: Debug.Assert(0);  /* cannot happen */
//   }
...
//   Debug.Assert(OPR_ADD <= opr && opr <= OPR_SHR);
...
//   Debug.Assert((VNIL <= e1->k && e1->k <= VKSTR) ||
//              e1->k == VNONRELOC || e1->k == VRELOC);
//   Debug.Assert(OP_ADD <= op && op <= OP_SHR);
...
//   OpCode op;
//   if (e1->k != VNONRELOC) {
//     Debug.Assert(e1->k == VK || e1->k == VKINT || e1->k == VKFLT);
//     swapexps(e1, e2);
//   }
