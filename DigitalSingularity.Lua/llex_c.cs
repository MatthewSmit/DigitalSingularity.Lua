...
public static unsafe partial class Lua
{
    ...
    private static void save(LexState* ls, int c)
    {
        ...
        if (luaZ_bufflen(b) + 1 > luaZ_sizebuffer(b))
        {
            ...
            luaZ_resizebuffer(ls->L, b, checked((int)newsize));
        }
...
//   luaD_throw(ls->L, LUA_ERRSYNTAX);
    ...
    */
    ...
    private static partial void luaX_setinput(lua_State* L, LexState* ls, Zio* z, TString* source, int firstchar)
    {
        ls->t.token = 0;
        ls->L = L;
...
//   Debug.Assert(ls->lookahead.token == TK_EOS);
